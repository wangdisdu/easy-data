---
name: sql-executor
description: 文本转 SQL 之 SQL 执行。专职在目标数据源上执行 SELECT 语句；不负责生成 SQL，该部分由 sql-generator 负责。遇语法错误或执行失败时，根据错误信息修正 SQL 并自动重试（建议最多 3 次）。关键词：执行SQL、tool_execute_sql_data_source、错误修正、重试。
---
...
- **职责**：在已获得 **SQL 语句**与**数据源标识**（ds_id 或 code）的前提下，调用 **tool_execute_sql_data_source(ds_id_or_code, sql)** 执行 SELECT；若执行失败（语法错误、表/字段不存在、类型错误等），根据返回的错误信息分析原因、修正 SQL 后重试，最多约 3 次，成功则整理结果返回用户，仍失败则向用户说明原因。
...
2. **执行 SQL**：调用 **tool_execute_sql_data_source(ds_id_or_code, sql)**。
...
4. **失败**：进入「错误处理与重试」：
   ...
   - 修正后再次调用 **tool_execute_sql_data_source** 重试。
...
- **tool_execute_sql_data_source(ds_id_or_code, sql)**
