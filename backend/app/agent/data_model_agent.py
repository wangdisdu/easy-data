"""
DataModelAgent - 数据模型管理Agent
使用LangGraph实现，专门处理数据模型相关的任务
...
from app.agent.data_model_agent_prompts import INTENT_ANALYSIS_PROMPT, MAIN_PROCESS_PROMPT
...
logger = get_logger("data_model_agent")
...
class DataModelAgentState(TypedDict):
    ...
    data_sources_info: Optional[str]  # 数据源列表信息


class DataModelAgent(BaseAgent):
    """数据模型管理Agent，使用LangGraph实现"""
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        2. 阶段二:主流程，支持查阅、导入、删除数据模型

        Returns:
            StateGraph: 编译后的工作流图
        """
        workflow = StateGraph(DataModelAgentState)

        # ========== 添加节点 ==========
        # 阶段一:入口阶段节点
        workflow.add_node("analyze_intent", self._analyze_intent_node)
        workflow.add_node("tool_list_ds", self.tool_node_list_ds)
        workflow.add_node("extract_ds_info", self._extract_data_sources_info_node)
        ...
        # 阶段一:工具执行后，提取数据源信息，然后进入主流程
        workflow.add_edge("tool_list_ds", "extract_ds_info")
        workflow.add_edge("extract_ds_info", "main_process")
    ...
    async def _analyze_intent_node(self, state: DataModelAgentState) -> DataModelAgentState:
        """
        阶段一节点:分析用户意图
    ...
    def _current_is_tool_call(self, state: DataModelAgentState) -> bool:
        """
        检查最新消息是否为工具调用
    ...
    async def _extract_data_sources_info_node(
        self, state: DataModelAgentState
    ) -> DataModelAgentState:
        """
        提取数据源列表信息节点

        从工具执行结果（ToolMessage）中提取数据源列表信息，保存到 state 中
        ...
        try:
            messages = state.get("messages", [])
            # 查找最后一个 ToolMessage（应该是 tool_list_data_source 的结果）
            for message in reversed(messages):
                if isinstance(message, ToolMessage):
                    # 提取数据源列表信息
                    data_sources_info = message.content
                    logger.info(
                        f"[EXTRACT-DS-INFO] 提取数据源列表信息，长度: {len(data_sources_info) if data_sources_info else 0}"
                    )
                    return {**state, "data_sources_info": data_sources_info}

            # 如果没有找到 ToolMessage，记录警告但继续
            logger.warning("[EXTRACT-DS-INFO] 未找到 ToolMessage，可能工具执行失败")
            return state
        except Exception:
            logger.exception("[EXTRACT-DS-INFO] [ERROR] 提取数据源列表信息失败")
            return state

    def _route_after_intent(self, state: DataModelAgentState) -> str:
        """
    ...
    async def _process_main_node(self, state: DataModelAgentState) -> DataModelAgentState:
        """
        阶段二节点:主流程处理

        处理查阅数据模型、导入数据模型、删除数据模型等操作。
        ...
        try:
            ...
            data_sources_info = state.get("data_sources_info")
            if data_sources_info:
                main_prompt = f"""{MAIN_PROCESS_PROMPT}
...
"""
    ...
    def _should_continue_main(self, state: DataModelAgentState) -> str:
        """
