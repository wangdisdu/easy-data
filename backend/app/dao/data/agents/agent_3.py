"""智能体初始化数据: [admin]数据模型分析助手（含本 agents 的节点与边）"""

import json

DESCRIPTION = """数据模型（数据库表/视图等）分析智能体，负责探索分析总结数据库表/视图等中的数据
"""
...
NODE2_CONFIG = {
    ...
    SYS_PROMPT = """你是一个数据模型分析助手，专门负责探索分析总结数据模型中的数据。
...
NODE5_CONFIG = {"tool_ids": [12]}

NODE6_CONFIG = {
    "script": """async def execute(state):
    import json
    ...
    if not isinstance(tool_message, ToolMessage) or not tool_message.content:
        return {**state, "model_info": None}
    ...
    if not result.get("id") or not result.get("name") or not result.get("code") or not result.get("platform") :
        return {**state, "model_info": None}

    model_info = {
        "dm_id_or_code": str(result.get("id")),
        "id": result.get("id"),
...
""",
    "route_mapping": ["model_not_found", "model_found"],
}
...
NODE9_CONFIG = {
    ...
    "script": '''async def execute(state, llm, tools):
    from langchain_core.messages import SystemMessage, AIMessage
    SYS_PROMPT = """你是一个SQL数据分析助手，为了了解目标模型的数据情况，专门负责执行多个数据分析SQL任务。
...
"""
    ...
    try:
        data = state.get("data")
        model_info = data.get("model_info")
        model_info_prompt = f"""## 数据模型信息（固定背景）
...
"""
        messages = state["messages"]
        messages = [SystemMessage(content=model_info_prompt), SystemMessage(content=SYS_PROMPT), *messages]
        response = await llm.ainvoke(messages)
        return {
            "messages": [response],
        }
...
NODE11_CONFIG = {"tool_ids": [16]}

NODE12_CONFIG = {
    "llm_id": 1,
    "script": '''async def execute(state, llm, tools):
    from langchain_core.messages import SystemMessage, AIMessage
    SYS_PROMPT = """你是一个数据模型分析助手，专门负责汇总所有SQL执行结果并为目标模型生成Markdown格式的数据分析报告。
...
NODE_ROWS = [
    ...
    },
    {
        ...
        "name": "数据分析SQL",
        "node_type": "llm",
        "config": json.dumps(NODE9_CONFIG),
        "description": None,
        "extend": None,
        "id": 27,
    },
    {
        ...
        "name": "条件分支",
        "node_type": "condition",
        "config": json.dumps(NODE10_CONFIG),
        "description": None,
        "extend": None,
        "id": 28,
    },
    {
        ...
        "name": "执行SQL",
        "node_type": "tool",
        "config": json.dumps(NODE11_CONFIG),
        "description": None,
        "extend": None,
        "id": 29,
    },
    {
        ...
        "name": "LLM分析总结",
        "node_type": "llm",
        "config": json.dumps(NODE12_CONFIG),
        "description": None,
        "extend": None,
        "id": 30,
    },
    {
        ...
        "name": "条件分支",
        "node_type": "condition",
        "config": json.dumps(NODE13_CONFIG),
        "description": None,
        "extend": None,
        "id": 31,
    },
    ...
    },
    {
        ...
        "name": "工具节点",
        "node_type": "tool",
        "config": json.dumps(NODE15_CONFIG),
        "description": None,
        "extend": None,
        "id": 33,
    },
